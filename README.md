...
`MainPageView` - главная страницы приложения, расширяет базовый компонент представления `Component<TMainPage>`. Основное
...
`MainPagePresenter` - класс, presenter для работы с главной страницей, расширяет базовый абстрактный класс `Presenter`.
...
`CatalogModel` через `MainPagePresenter` посредством запроса на backend API (`GET /product/`).
...
Это событие обрабатывает `MainPagePresenter`, который выполняет render галереи в представлении каталога товаров
...
в корзине (необходимо для корректного отображения состояния кнопки карточки), далее через `MainPagePresenter` происходит
...
После обновления модели корзины генерируется событие `cart:content_changed` и обрабатывается в
соответствующем [сценарии](#изменение-состояния-корзины).
...
* `CartPresenter`, который обновление представления корзины `CartView`. Если в корзине нет товаров, то кнопка "Оформить" неактивна.
* `ApplicationPresenter`, который обновляет содержимое текущего расширенного представления карточки товара
  `CardPreview`.
* `MainPagePresenter`, который обновляет общее количество товаров в корзине на главной странице.
...
`MainPagePresenter` так же через событие вызывает блокировку главной страницы `MainPageView`.

### Открытие модального окна

При открытии модального окна, генерируется событие `modal:opened` с пустым телом, на которое подписан
`MainPagePresenter`, который вызывает блокировку главной страницы `MainPageView`.

### Закрытие модального окна

При закрытии модального окна, генерируется событие `modal:closed` с пустым телом, на которое подписан `MainPagePresenter`.
Вызывает разблокировку главной страницы `MainPageView`.
