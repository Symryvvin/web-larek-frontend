...
`Card` - базовая карточка товара, расширяющая `Component<Product>`:

* `productId: ProductId` - идентификатор товара и setter для данного элемента.
...
`GalleryCard` - базовая карточка галереи товара, расширяющая `Card` дополнительными компонентами:
...
`CatalogCard` - класс карточки товара в галереи каталога, расширяет `GalleryCard`,
создается по шаблону `'#card-catalog'`

`CardPreview` - класс карточки товара для отображения полного описания товара, расширяет `GalleryCard`, создается по
шаблону `'#card-preview'`:
...
`CardInCart` - класс карточки товара для отображения полного описания товара, расширяет `GalleryCard`, создается по
шаблону `'#card-basket'`:
...
* **'app:failed'** - произошла ошибка в приложении.