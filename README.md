...
`ICartModel` - интерфейс для модели корзины. Корзина содержит коллекцию из идентификаторов товаров `ProductId`. С корзиной можно осуществить следующие действия:
...
* ApplicationPresenter - основное презентер приложения
* CartPresenter - презентер для работы с корзиной.
* CatalogPresenter - презентер для работы с каталогом товаров.
...
Наименование события представляет собой строку в формате "snake case", где через двоеточие указан контекст событие и его название, определяющее, что произошло:
`context:example_name`

### Загрузка главного окна приложения

При входе пользователя на главную страницу приложения через модель каталога `CatalogModel` происходит запрос на сервер для получения списка товаров. После успешного получения списка товаров, он записывается в коллекцию модели каталога и генерируется событие `catalog:items_loaded` с данными в виде списка items (товаров) полученных из модели `CatalogModel`. 

На это событие подписан `MainPage` для построения галереи в каталоге товаров на html-странице. Для списка загруженных товаров из модели создаются представления по шаблону `CardInCatalog` и добавляются массивом html-элементов в `Catalog`.

Из локального хранилища корзины с ключом `cart` с товарами подгружается список товаров в корзине в модель и генерируется событие `cart:content_changed`.

### Нажатие на карточку с товаров

При нажатии на карточку товара генерируется событие `catalog:card_selected`, куда передаются данные выбранной карточки товара. Событие обрабатывается в `Catalog`, где с соответствующим контентом по шаблону `CardPreview` открывается детальная карточка товара.

### Добавление товара в корзину

При нажатии на кнопку "В корзину" генерируется событие `cart:item_added`, куда передается идентификатор товара добавленного в корзину. На событие подписана модель корзины `CartModel` в которой обновляется модель и локальное хранилище корзины по ключу `cart`. После обновления модели корзины генерируется событие `cart:content_changed`.

### Удаление товара из корзины

При нажатии на кнопку удаления товара из корзины генерируется событие `cart:item_deleted`, куда передается идентификатор товара удаляемого из корзины. 

На событие подписана модель корзины `CartModel`, в которой обновляется модель и локальное хранилище корзины по ключу `cart`. После обновления модели корзины генерируется событие `cart:content_changed`.

### Изменение состояния корзины

При любом изменении корзины, добавление, удаление товара или успешное оформление заказа генерируется событие `cart:content_changed` с телом в виде актуального списка идентификаторов товаров в корзине.

Если в данное время модульное окно с содержимым корзины открыто, то происходит обновление представления корзины `Cart`. Обновление общего количество товаров на главной страницы происходит всегда в `MainPage`, благодаря тому что эти представления подписаны на данное событие.

### Открытие корзины

При нажатии на иконку корзины, открывается модальное окно `Modal`, в которое в качестве контента передается `CartView` со списком элементов `CardInCart` полученных из модели корзины.

### Оформление заказа

При нажатии на кнопку оформления заказа из данных модели корзины создается объект `Order` с исходными данными в виде списка идентификаторов товаров и общего количество оформляемых товаров. Объект оформляемого заказа хранится в модели корзины. После успешного создания заказа, текущее модальное окно закрывается и генерируется событие `order:created` с данными в виде объекта `Order`, на которое подписана главная страница `MainPage`, для того чтобы открыть новое модальное окно с формой оформления заказа `OrderForm`.

При успешной отправки формы оформления заказа, объект `Order` дополняется данными: способ оплаты и адрес доставки. Проверяется корректное заполнение формы и после успешного оформления заказа текущее модальное окно закрывается и генерируется новое событие `order:payment_selected`, в которое передается обновленный объект `Order`. На событие подписана главная страница `MainPage`, для того чтобы открыть новое модальное окно с формой заполнения контактных данных `ContactsForm`.

При успешной отправки формы с контактными данными, объект `Order` дополняется данными: телефон и адрес доставки.
Проверяется корректное заполнение формы и после успешного заполнения формы заказ через API отправляется на сервер. . После успешной отправки заказа на сервер текущее модальное окно закрывается и генерируется новое событие `order:placed`.

На событие подписана главная страница `MainPage`, которая открывает модальное окно с информацией об успешном оформлении заказа `OrderSuccess` со значением списанных с баланса средств.
