...
`CardInCatalog` - класс карточки товара в галереи каталога, расширяет базовый компонент представления `Component<Product>`, создается по шаблону `'#card-catalog'`:
* `readonly id: ProductId` - идентификатор товара для быстрого доступа к нему только для чтения, передается в конструкторе класса.
...
* `addToCartButton: HTMLButtonElement` - кнопка покупки товара.
