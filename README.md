...
При нажатии на кнопку карточки товара `CardPreview` генерируется событие `cart:item_added`, куда передается идентификатор товара добавляемого в корзину. На событие подписан `ApplicationPresenter`, который, через `MainPagePresenter` ищет товара по идентификатору в модели каталога `CatalogModel` и добавляет товар в модель корзины  `CartModel`. Так же обновляется локальное хранилище в браузере по ключу `cart`. 
